...
function prepareGameField() {
    currentScore = document.querySelector(
        'h3'); //Переменная для постоянного отображения текущего счета
...
function move() {
    ...
    //     new_unit = document.getElementsByClassName('cell-' + (coord_y) + '-' + (0))[
    //         0]; ///----------------------------------
    ...
    function coordXY(newUnit) {
        return function (arg1, arg2, arg3, arg4) {
            ...
            } else if (new_unit == newUnit && direction == arg3) { //Изменение координаты направления по оси у вниз
                new_unit = document.getElementsByClassName('cell-' + (FIELD_SIZE_Y - 1) + '-' + (coord_x))[0];
            } else if (new_unit == newUnit && direction == arg4) { //Изменение координаты направления по оси у вверх
                new_unit = document.getElementsByClassName('cell-' + (0) + '-' + (coord_x))[0];
            }
        }
    }
    var s = coordXY(undefined);
    s('x-', 'x+', 'y+', 'y-');
